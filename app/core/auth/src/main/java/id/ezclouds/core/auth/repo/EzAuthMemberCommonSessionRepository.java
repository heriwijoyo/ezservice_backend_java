...
public interface EzAuthMemberCommonSessionRepository extends JpaRepository<EzAuthMemberCommonSessionDO, String> {
    ...
    @Query("SELECT cs FROM EzAuthMemberCommonSessionDO cs WHERE cs.orgId = :orgId AND cs.scene = :scene AND cs.verifyStrategy = :verifyStrategy AND cs.memberId = :memberId")
    List<EzAuthMemberCommonSessionDO> findBySceneLoginId(
